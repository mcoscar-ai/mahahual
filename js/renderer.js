...
function updateGroundY() {
  GROUND_Y = CANVAS.height - Math.round(CANVAS.height * 0.12);
  P.y = Math.min(P.y, GROUND_Y); // garante que não fica flutuando após resize
}
...
function resizeCanvas() {
  ...
  var scale  = Math.min(scaleX, scaleY); // preserva proporção (igual ao Gabriel)
  ...
  updateGroundY();
  if (typeof layoutBtns === 'function') layoutBtns(); // reposiciona botões touch
}
